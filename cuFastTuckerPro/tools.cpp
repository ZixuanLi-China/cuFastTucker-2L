...
void Parameter_Initialization(int order, int core_kernel, int core_length,
		int *dimen, double data_norm, type_of_data ***parameter_cp) {

	*parameter_cp = (type_of_data**) malloc(sizeof(type_of_data*) * order);

	for (int i = 0; i < order; i++) {
		(*parameter_cp)[i] = (type_of_data*) malloc(
				sizeof(type_of_data) * dimen[i] * core_kernel);
	}

	for (int i = 0; i < order; i++) {
		for (int j = 0; j < dimen[i] * core_kernel; j++) {
			(*parameter_cp)[i][j] = pow(data_norm / core_kernel, 1.0 / order) * frand(0.5, 1.5);
		}
	}
}
...
void Cuda_Parameter_Initialization(int order, int core_kernel, int core_dimen,
	...
	*value_train_host_to_device = (type_of_data**) malloc(
	...
	for (int i = 0; i < order; i++) {
		...
		cudaMemcpy(temp_cp, parameter_cp_host[i],
				sizeof(type_of_data) * dimen[i] * core_kernel,
